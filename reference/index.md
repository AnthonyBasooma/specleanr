# Package index

## All functions

- [`abdata`](https://anthonybasooma.github.io/specleanr/reference/abdata.md)
  : Alburnoides bipunctatus species data from GBIF and iNaturalist

- [`adjustboxplots()`](https://anthonybasooma.github.io/specleanr/reference/adjustboxplots.md)
  : Adjust the boxplots bounding fences using medcouple to flag
  suspicious outliers.

- [`bestmethod()`](https://anthonybasooma.github.io/specleanr/reference/bestmethod.md)
  : Identifies the best method for outlier detection for a single
  species.

- [`boots()`](https://anthonybasooma.github.io/specleanr/reference/boots.md)
  : To implement bootstrapping procedures. Sampling with replacement.

- [`broad_classify()`](https://anthonybasooma.github.io/specleanr/reference/broad_classify.md)
  : Outlier detection method broad classification.

- [`check.exclude()`](https://anthonybasooma.github.io/specleanr/reference/check.exclude.md)
  : indicate excluded columns.

- [`check_names()`](https://anthonybasooma.github.io/specleanr/reference/check_names.md)
  : Check species names for inconsistencies

- [`check_packages()`](https://anthonybasooma.github.io/specleanr/reference/check_packages.md)
  : Check for packages to install and respond to use

- [`checks()`](https://anthonybasooma.github.io/specleanr/reference/checks.md)
  : Post checks for PCA and bootstrapping

- [`classify_data()`](https://anthonybasooma.github.io/specleanr/reference/classify_data.md)
  : Extract final clean data using either absolute or best method
  generated outliers.

- [`cosine()`](https://anthonybasooma.github.io/specleanr/reference/cosine.md)
  : Cosine similarity index based on (Gautam & Kulkarni 2014; Joy &
  Renumol 2020)

- [`datacleaner-class`](https://anthonybasooma.github.io/specleanr/reference/datacleaner-class.md)
  : Outlier detection class for multiple methods

- [`distboxplot()`](https://anthonybasooma.github.io/specleanr/reference/distboxplot.md)
  : Distribution boxplot

- [`ecological_ranges()`](https://anthonybasooma.github.io/specleanr/reference/ecological_ranges.md)
  : Check for environmental outliers using species optimal ranges.

- [`efidata`](https://anthonybasooma.github.io/specleanr/reference/efidata.md)
  : EFIPLUS data used to develop ecological sensitivity parameters for
  riverine species in European streams and rivers.

- [`eif()`](https://anthonybasooma.github.io/specleanr/reference/eif.md)
  : Computes the empirical influence function for each values in the
  dataset

- [`extentvalues()`](https://anthonybasooma.github.io/specleanr/reference/extentvalues.md)
  : To check for a bounding box

- [`extractMethods()`](https://anthonybasooma.github.io/specleanr/reference/extractMethods.md)
  : List of outlier detection methods implemented in this package.

- [`extract_clean_data()`](https://anthonybasooma.github.io/specleanr/reference/extract_clean_data.md)
  : Extract final clean data using either absolute or best method
  generated outliers.

- [`extractoutliers()`](https://anthonybasooma.github.io/specleanr/reference/extractoutliers.md)
  : Extract outliers for a one species

- [`geo_ranges()`](https://anthonybasooma.github.io/specleanr/reference/geo_ranges.md)
  : Checks for geographic ranges from FishBase

- [`getdata()`](https://anthonybasooma.github.io/specleanr/reference/getdata.md)
  : Download species records from online database.

- [`getdiff()`](https://anthonybasooma.github.io/specleanr/reference/getdiff.md)
  : get dataframe from the large dataframe.

- [`ggenvironmentalspace()`](https://anthonybasooma.github.io/specleanr/reference/ggenvironmentalspace.md)
  : Title Plotting to show the quality controlled data in environmental
  space.

- [`ggoutlieraccum()`](https://anthonybasooma.github.io/specleanr/reference/ggoutlieraccum.md)
  : Identify if enough methods are selected for the outlier detection.

- [`ggoutliers()`](https://anthonybasooma.github.io/specleanr/reference/ggoutliers.md)
  : Visualize the outliers identified by each method

- [`hamming()`](https://anthonybasooma.github.io/specleanr/reference/hamming.md)
  : Identify best outlier detection method using Hamming distance.

- [`hampel()`](https://anthonybasooma.github.io/specleanr/reference/hampel.md)
  : Flag suspicious outliers based on the Hampel filter method..

- [`handle_true_errors()`](https://anthonybasooma.github.io/specleanr/reference/handle_true_errors.md)
  : Catch errors during methods implementation.

- [`interquartile()`](https://anthonybasooma.github.io/specleanr/reference/interquartile.md)
  : Computes interquartile range to flag environmental outliers

- [`isoforest()`](https://anthonybasooma.github.io/specleanr/reference/isoforest.md)
  : Identify outliers using isolation forest model.

- [`jaccard()`](https://anthonybasooma.github.io/specleanr/reference/jaccard.md)
  : Identifies the best outlier detection method using Jaccard
  coefficient.

- [`jdsdata`](https://anthonybasooma.github.io/specleanr/reference/jdsdata.md)
  : Joint Danube Survey Data

- [`jknife()`](https://anthonybasooma.github.io/specleanr/reference/jknife.md)
  : Identifies outliers using Reverse Jackknifing method based on
  Chapman et al., (2005).

- [`kdat`](https://anthonybasooma.github.io/specleanr/reference/kdat.md)
  : Sequential fences constants

- [`logboxplot()`](https://anthonybasooma.github.io/specleanr/reference/logboxplot.md)
  : Log boxplot based for outlier detection.

- [`mahal()`](https://anthonybasooma.github.io/specleanr/reference/mahal.md)
  : Flags outliers based on Mahalanobis distance matrix for all records.

- [`match.argc()`](https://anthonybasooma.github.io/specleanr/reference/match.argc.md)
  : Customized match function

- [`match_datasets()`](https://anthonybasooma.github.io/specleanr/reference/match_datasets.md)
  : Data harmonizing for offline data based on Darwin Core terms .

- [`medianrule()`](https://anthonybasooma.github.io/specleanr/reference/medianrule.md)
  : Median rule method

- [`mixediqr()`](https://anthonybasooma.github.io/specleanr/reference/mixediqr.md)
  :

  Mixed Interquartile range and semiInterquartile range
  `Walker et al., 2018`

- [`mth`](https://anthonybasooma.github.io/specleanr/reference/mth.md) :
  mth datasets with constant at each confidence interval levels.

- [`multiabsolute()`](https://anthonybasooma.github.io/specleanr/reference/multiabsolute.md)
  : Identifies absolute outliers for multiple species.

- [`multibestmethod()`](https://anthonybasooma.github.io/specleanr/reference/multibestmethod.md)
  : Identify best method for outlier removal for multiple species using
  majority votes.

- [`multidetect()`](https://anthonybasooma.github.io/specleanr/reference/multidetect.md)
  : Ensemble multiple outlier detection methods.

- [`ocindex()`](https://anthonybasooma.github.io/specleanr/reference/ocindex.md)
  : Identifies absolute outliers and their proportions for a single
  species.

- [`onesvm()`](https://anthonybasooma.github.io/specleanr/reference/onesvm.md)
  : Identify outliers using One Class Support Vector Machines

- [`optimal_threshold()`](https://anthonybasooma.github.io/specleanr/reference/optimal_threshold.md)
  : Optimize threshold for clean data extraction.

- [`overlap()`](https://anthonybasooma.github.io/specleanr/reference/overlap.md)
  : Identifies best outlier detection method using Overlap coefficient.

- [`pca()`](https://anthonybasooma.github.io/specleanr/reference/pca.md)
  : Implement principal component analysis for dimension reduction

- [`pcboot()`](https://anthonybasooma.github.io/specleanr/reference/pcboot.md)
  : To package both principal component analysis and bootstrapping.

- [`pred_extract()`](https://anthonybasooma.github.io/specleanr/reference/pred_extract.md)
  : Preliminary data cleaning including removing duplicates, records
  outside a particular basin, and NAs.

- [`search_threshold()`](https://anthonybasooma.github.io/specleanr/reference/search_threshold.md)
  : Determine the threshold using Locally estimated or weighted
  Scatterplot Smoothing.

- [`semiIQR()`](https://anthonybasooma.github.io/specleanr/reference/semiIQR.md)
  : Computes semi-interquantile range to flag suspicious outliers

- [`seqfences()`](https://anthonybasooma.github.io/specleanr/reference/seqfences.md)
  : Sequential fences method

- [`show(`*`<datacleaner>`*`)`](https://anthonybasooma.github.io/specleanr/reference/show-datacleaner-method.md)
  : set method for displaying output details after outlier detection.

- [`smc()`](https://anthonybasooma.github.io/specleanr/reference/smc.md)
  : Identify best outlier detection method using simple matching
  coefficient.

- [`sorensen()`](https://anthonybasooma.github.io/specleanr/reference/sorensen.md)
  : Identifies best outlier detection method suing Sorensen Similarity
  Index.

- [`thermal_ranges()`](https://anthonybasooma.github.io/specleanr/reference/thermal_ranges.md)
  : Collates minimum, maximum, and preferable temperatures from
  FishBase.

- [`ttdata`](https://anthonybasooma.github.io/specleanr/reference/ttdata.md)
  : Thymallus thymallus species data from GBIF and iNaturalist

- [`xglosh()`](https://anthonybasooma.github.io/specleanr/reference/xglosh.md)
  : Global-Local Outlier Score from Hierarchies

- [`xkmeans()`](https://anthonybasooma.github.io/specleanr/reference/xkmeans.md)
  : Flags outliers using kmeans clustering method

- [`xknn()`](https://anthonybasooma.github.io/specleanr/reference/xknn.md)
  : k-nearest neighbors for outlier detection

- [`xlof()`](https://anthonybasooma.github.io/specleanr/reference/xlof.md)
  : Flags suspicious using the local outlier factor or Density-Based
  Spatial Clustering of Applications with Noise.

- [`zscore()`](https://anthonybasooma.github.io/specleanr/reference/zscore.md)
  : Computes z-scores to flag environmental outliers.
